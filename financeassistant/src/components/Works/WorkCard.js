...
function WorkCard(props) {
    ...
    return(
        <Card className={"mx-auto card mb-3"} style={{ width: '18rem',margin:20,backgroundColor: 'rgba(244, 0, 0, 0.5)',color:'#000000'}}>
            <Card.Body>
                <Card.Title>Name: <Link to={`/works/edit/${props.item.id}`} style={{color:'#000000'}}> {props.item.name}</Link></Card.Title>
                <Card.Text style={{color:'#000000'}}>
                    <b>Address: {props.item.address}</b>
                    <br/>
                    <b>Number: {props.item.number}</b>
                </Card.Text>
